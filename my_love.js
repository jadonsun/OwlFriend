...
var MyLoveListInfo = function() {
}
MyLoveListInfo.prototype = {

    init: function() {
        var self = this;
        self.query_my_love_size();
    },

    query_my_love_size: function() {
        var req_args = [];
        var func_name = "query_youLike_list_size";
        window.postMessage({
    ...
    query_like_list_by_page: function(cur_page) {
        var req_args = [];
        var func_name = "query_youLike_list_by_page";
        ...
        window.addEventListener('message', function(e) {
            ...
            if(e.data && e.data.data && e.data.data.neb_call) {
                ...
                if(e.data.data.neb_call.result) {
                    ...
                    if (obj.type == "query_youLike_list_by_page") {
                        $("#loading_page").hide();
                        $("#page_detail").show();
                        ...
                        if (obj.success == false) {
                            $("#my_love_list").hide();
                            $("#no_data_my_love").show();
                            
                        } else {
                            $("#no_data_my_love").hide();
                            $("#my_love_list").show();
                        }
                        self.parse_youlike_list(obj.data);
                    } else if(obj.type == "query_youLike_list_size") {
                        if (obj.success == false) {
                            $("#love_list_loading").hide();
                            $("#my_love_list").hide();
                            $("#no_data_my_love").show();
                        ...
                        self.parse_youlike_num(obj.list_size);
                    }
                } else {
    ...
    parse_youlike_list: function(youlike_list) {
        if (youlike_list.length == 0) {
            // 没有数据
        } else {
            $("#my_love_list").empty().show();
            // 显示内容
            var my_love_list_t = template(document.getElementById('person_list_t').innerHTML);
            var my_love_list_html = my_love_list_t({list: youlike_list});
            $("#my_love_list").append(my_love_list_html);
        }
    },

    parse_youlike_num: function(total_num) {
        if (total_num == 0) {
            $("#loading_page").hide();
            $("#page_detail").show();
            return;
...
Pagination.prototype = {
    ...
    onChoosePageEvent: function(click_page_num, list_offset) {
        ...
        myLoveListObj.query_like_list_by_page(click_page_num);
    },
}
...
function checkNebpay() {
    ...
    // 环境ok，拉取数据
    myLoveListObj = new MyLoveListInfo();
    myLoveListObj.init();
    myLoveListObj.listenWindowMessage();
